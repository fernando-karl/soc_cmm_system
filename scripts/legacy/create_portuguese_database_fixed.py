#!/usr/bin/env python3
"""
Script para criar banco de dados SOC CMM em português com schema correto
...
def create_database_schema(db_path: str):
    ...
    cursor.execute('''
        CREATE TABLE questions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            aspect_id VARCHAR(100) NOT NULL,
        ...
        if isinstance(domain_data, dict):
            for aspect_name, aspect_data in domain_data.items():
                if isinstance(aspect_data, list):
                    ...
                    aspect_id = f"{domain_name}.{aspect_name}"
                    cursor.execute("SELECT id FROM aspects WHERE id = ?", (aspect_id,))
                        ...
                        for question in aspect_data:
                            if isinstance(question, dict) and 'question' in question:
                                ...
                                cursor.execute(
                                    ...
                                    (aspect_db_id, question_text, field_type, question_counter, guidance)
                                )
